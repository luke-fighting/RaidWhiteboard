...
local function SendRaw(message)
    local channel
    if RWB:IsInRaidGroup() then
        channel = "RAID"
    elseif RWB:IsInPartyGroup() then
        channel = "PARTY"
    else
        return
    end
...
local function SerializeStroke(data)
    ...
    for _, p in ipairs(data.points) do
        table.insert(points, math.floor(p.x + .5) .. "," .. math.floor(p.y + .5))
    end

    local c = data.color
    return c.r .. "," .. c.g .. "," .. c.b .. "," .. (c.a or 1) .. "," ..
        data.thickness .. "|" .. table.concat(points, ";")
end
...
local function DeserializeStroke(payload)
    ...
    return {
        ...
        thickness = tonumber(width),
        tool = "PEN"
    }
end

function RWB:BroadcastBoardState(open)
...
function RWB:BroadcastCurrentCanvas()
    ...
    for id, data in pairs(self.strokes) do
        self:BroadcastStroke(id, data, true)
    end

    for id, data in pairs(self.texts) do
        self:BroadcastText(id, data, true)
    end
end

function RWB:BroadcastStroke(id, data, force)
    if not force and not self.syncEnabled then return end

    local payload = SerializeStroke(data)

    SendRaw("S:" .. id)

    for index = 1, #payload, CHUNK_SIZE do
        SendRaw("C:" .. id .. ":" .. payload:sub(index, index + CHUNK_SIZE - 1))
    end

    SendRaw("E:" .. id)
end

function RWB:BroadcastRemoveStroke(id, force)
    if not force and not self.syncEnabled then return end
    SendRaw("R:" .. id)
end

function RWB:BroadcastText(id, data, force)
    if not force and not self.syncEnabled then return end
...
function RWB:BroadcastRemoveText(id, force)
    if not force and not self.syncEnabled then return end
    SendRaw("TR:" .. id)
end

function RWB:BroadcastClear(force)
    if not force and not self.syncEnabled then return end
    SendRaw("X")
end

function RWB:RequestSync()
    SendRaw("Q")
end

function RWB:OnComm(message)
    -- X is always a remote replacement/clear operation.
    if message == "X" then
    ...
    if message == "Q" then
        -- Only lead/assist responds with the authoritative presentation state.
    ...
    elseif kind == "C" then
        ...
        if incoming[id] then
            table.insert(incoming[id], chunk)
        end
    ...
    elseif kind == "E" then
        ...
        if chunks then
            ...
            if data then
                self:RenderRemoteStroke(rest, data)
            end
    ...
    elseif kind == "T" then
        local id,x,y,r,g,b,text =
            rest:match("^(.-):([^,]+),([^,]+),([^,]+),([^,]+),([^|]+)|(.+)$")

        if id then
            self:RenderRemoteText(id, {
                x = tonumber(x),
                y = tonumber(y),
                text = text,
                color = {
